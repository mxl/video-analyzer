...
def create_client(config: Config):
    ...
    if client_type == "ollama":
        ...
        return GenericOpenAIAPIClient(
            client_config["api_key"], client_config["api_url"]
        )
    else:
        raise ValueError(f"Unknown client type: {client_type}")


def resolve_max_frames(args: argparse.Namespace, config: Config) -> int:
    """Resolve max_frames from CLI first, then config, then fallback."""
    if args.max_frames is not None:
        return args.max_frames
    return config.get("frames", {}).get("max_frames", sys.maxsize)
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, help="Output directory for analysis results"
    )
    parser.add_argument(
        "--client", type=str, help="Client to use (ollama or openrouter)"
    )
    parser.add_argument("--ollama-url", type=str, help="URL for the Ollama service")
    parser.add_argument(
        "--api-key", type=str, help="API key for OpenAI-compatible service"
    )
    ...
    parser.add_argument(
        "--whisper-model",
        type=str,
        help="Whisper model size (tiny, base, small, medium, large), or path to local Whisper model snapshot",
    )
    parser.add_argument(
        "--start-stage",
        type=int,
        default=1,
        help="Stage to start processing from (1-3)",
    )
    parser.add_argument(
        "--max-frames",
        type=int,
    ...
    parser.add_argument(
        "--prompt", type=str, default="", help="Question to ask about the video"
    )
    ...
    max_frames = resolve_max_frames(args, config)
    ...
    try:
        ...
        if args.start_stage <= 1:
            ...
            if audio_path is None:
                ...
                if transcript is None:
                    logger.warning(
                        "Could not generate reliable transcript. Proceeding with video analysis only."
                    )
        ...
        logger.info(
            f"Analysis complete. Results saved to {output_dir / 'analysis.json'}"
        )
