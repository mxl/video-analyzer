...
class PromptLoader:
    ...
    def _find_prompt_file(self, prompt_path: str) -> Path:
        ...
        # Prefer a user-specified prompt directory when present so custom prompts
        # can override packaged defaults.
        if self.prompt_dir:
        ...
        try:
            package_path = pkg_resources.resource_filename(
                "video_analyzer", f"prompts/{prompt_path}"
            )
    ...
    def get_by_index(self, index: int) -> str:
        ...
        try:
            if index < 0 or index >= len(self.prompts):
                raise IndexError(
                    f"Prompt index {index} out of range (0-{len(self.prompts) - 1})"
                )
